...
install_docker_apt() {
    log "Installing Docker via apt (Debian/Ubuntu)..."
    apt-get update -qq
...
if ! command -v docker &>/dev/null; then
    ...
    # Try convenience script first; fall back to manual apt if distro not detected
    if curl -fsSL https://get.docker.com | sh 2>&1 | grep -q "Unsupported distribution"; then
        warn "Convenience script failed — trying manual apt install..."
        install_docker_apt
    fi
